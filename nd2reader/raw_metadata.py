...
class RawMetadata(object):
    ...
    def _parse_roi(self, raw_roi_dict):
        ...
        roi_dict["timepoints"] = np.array(roi_dict["timepoints"], dtype=float)
        roi_dict["positions"] = np.array(roi_dict["positions"], dtype=float)
        roi_dict["sizes"] = np.array(roi_dict["sizes"], dtype=float)
